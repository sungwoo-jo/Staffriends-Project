...
async function getSampleImage(i) { // 샘플 이미지 가져오기
            let imagePath = "/img/road_sample_img/road_sample_img_" + i + ".jpg";
            imgElement.src = imagePath;
}

function doGuardPage(serialNum) { // 보호자 페이지로 이동
    let form = document.createElement('form'); // form element 생성
    let obj = document.createElement('input'); // input element 생성
    ...
    form.appendChild(obj); // form element의 자식 요소로 obj(input element)를 추가
    form.setAttribute('method', 'post'); // method를 post 방식으로,
    form.setAttribute('action', '/guard/guardIndex'); // action은 /guardIndex로 전송되도록
    document.body.appendChild(form); // body의 자식 요소로 form을 추가
    form.submit(); // form 전송
}